...
Begin VB.Form frmMain 
   ...
   Caption         =   "rion Belt : The World's Best VB Code Commenter"
   ClientHeight    =   4800
   ClientLeft      =   2205
   ClientTop       =   4395
   ClientWidth     =   8280
...
Private Sub cmdBrowse_MouseUp(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    cdlBrowse.ShowOpen
    strTargetProject = cdlBrowse.FileName
    txtTarget = strTargetProject + Space$(50)
...
Private Sub cmdComment_MouseUp(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    If Dir$(strTargetProject) <> "" Then 'If the file exist then...
        If LCase$(Right(strTargetProject, 3)) = "vbp" Then 'If the file match the vb project extension then...
...
Private Sub timColorScroll_Timer()
    Static bytCurColor As Byte 'Declare it as byte as the only numbers involve is 0 and 1
    
    'Check if it's time to switch the foreground layer
    If Len(lblOrionBelt(bytCurColor)) = 13 Then
        bytCurColor = 1 - bytCurColor
        lblOrionBelt(bytCurColor).ZOrder
        lblOrionBelt(bytCurColor) = Space$(4)
    End If
    
    'And now we add 1 more character to current layer
    lblOrionBelt(bytCurColor) = lblOrionBelt(bytCurColor) + Mid$("    rion Belt", Len(lblOrionBelt(bytCurColor)) + 1, 1)
    
    'That's all!
    'At first I thought it's gonna be hard to write this code =O]
End Sub
